...
def forward_message(message, next_host, next_port):
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            print(f"[SERVER-0] Message forwarded successfully to {next_host}:{next_port}")
    except Exception as e:
        print(f"[SERVER-0] Forward failed: {e}")
...
def start_server(port, next_hostport):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
        ...
        server.listen()
        print(f"[SERVER-0] Listening on port {port}, will forward to {next_hostport}")
        conn, addr = server.accept()
        with conn:
            data = conn.recv(1024).decode('utf-8')
            print(f"[SERVER-0] Received message from {addr[0]}:{addr[1]}: {data}")
